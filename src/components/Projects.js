...
const Projects = () => {
  ...
  const autoSyncGithubRepos = useCallback(async () => {
    if (!githubRepoService.isTokenConfigured()) {
      console.warn("GitHub token not configured. Skipping GitHub sync.");
      return;
    }
    
    const username = process.env.REACT_APP_GITHUB_USERNAME || 'devranbir';
    ...
    try {
      ...
      const repos = await githubRepoService.getRepositoriesAsProjects(username, githubImportOptions);
      
      if (repos.length === 0) {
        console.log("No GitHub repositories found or all repositories were filtered out.");
        showMessage("No new GitHub repositories to import.");
        return;
      }
      
      // Filter out repos that are already in projects (by repo URL or name)
      const existingRepoUrls = new Set(projects.map(p => p.repoUrl).filter(url => url));
      const existingNames = new Set(projects.map(p => p.name));
      
      const newRepos = repos.filter(repo => 
        !existingRepoUrls.has(repo.repoUrl) && 
        !existingNames.has(repo.name)
      );
      
      if (newRepos.length === 0) {
        console.log("All GitHub repositories are already imported.");
        return;
      }
      
      // Generate new IDs for the repositories
      const maxId = Math.max(...projects.map(p => p.id), 0);
      const reposWithIds = newRepos.map((repo, index) => ({
        ...repo,
        id: maxId + index + 1
      }));
      
      // Add all new repositories
      const updatedProjects = [...projects, ...reposWithIds];
      setProjects(updatedProjects);
      ...
      showMessage(`Successfully added Github project(s)!`);
      console.log(`Successfully synced ${newRepos.length} new repositories from GitHub!`);
    } catch (error) {
      showMessage(`Error in adding GitHub project(s)! ${error.message}`);
      console.error(`Error syncing with GitHub: ${error.message}`);
    } finally {
      showMessage("GitHub repositories sync completed.");
    }
  }, [projects]);
  
  // Auto-sync GitHub repositories when component mounts
  useEffect(() => {
    let isMounted = true;
    
    const initialSync = async () => {
      // Wait a bit for projects to load first
  ...
  }, [autoSyncGithubRepos]); // Include autoSyncGithubRepos as dependency
  
  // Auto-sync GitHub repositories periodically (every 30 minutes)
  useEffect(() => {
